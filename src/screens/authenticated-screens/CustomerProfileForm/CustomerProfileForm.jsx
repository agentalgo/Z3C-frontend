...
function CustomerProfileForm() {
  ...
  const handleSubmitForm = (e) => {
    ...
    // Shape the payload to be close to the sample from customer-profile-payload.txt
    const payload = {
    ...
    // Sample submit behaviour for now – wire to API later
    // eslint-disable-next-line no-console
    console.log('Customer profile payload (sample):', payload);
    showToast('Customer profile data prepared (sample).', 'success');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            value={formData.data.defaultTemplate}
            onChange={handleChangeFormData}
            placeholder="e.g. b2b-invoice-template"
            className="px-4 py-2.5 rounded-lg border border-[#e7ebf3] dark:border-[#2a3447] bg-white dark:bg-[#161f30] text-sm text-[#0d121b] dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors"
          />
        </div>
      </div>
